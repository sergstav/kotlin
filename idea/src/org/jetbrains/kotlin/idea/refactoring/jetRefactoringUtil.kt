...
// Can be either PsiClass or PsiEnumConstant
fun createJavaClass(klass: JetClass, targetClass: PsiClass?): PsiMember {
    ...
    val javaClass = targetClass?.add(javaClassToAdd) as? PsiClass ?: javaClassToAdd
    ...
    for (method in template.getMethods()) {
        val hasConstructorParams = method.getParameterList().getParametersCount() > 0
        ...
        if (method.isConstructor() && !(hasConstructorParams || needSuperCall)) continue
